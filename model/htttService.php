...
{
   return httt_Get("SELECT httt.id, httt.name, httt.content, httt.position FROM  httt ".(($where!='')?(' where '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function httt_insert($obj)
{
    return exe_query("insert into httt (name,content,position) values ('$obj->name','$obj->content','$obj->position')",'httt');
}
//
function httt_update($obj)
{
    return exe_query("update httt set name='$obj->name',content='$obj->content',position='$obj->position' where id=$obj->id",'httt');
}
//
