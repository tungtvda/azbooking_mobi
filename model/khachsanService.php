...
{
   return khachsan_Get("SELECT khachsan.id, danhmuc_khachsan.name as danhmuc_id,  danhmuc_khachsan_2.name as danhmuc2_id, khachsan.highlights, khachsan.name, khachsan.name_url, khachsan.start, khachsan.price, khachsan.dichvu, khachsan.img, khachsan.address, khachsan.phone, khachsan.email, khachsan.map, khachsan.content, khachsan.title, khachsan.keyword, khachsan.description FROM  khachsan, danhmuc_khachsan, danhmuc_khachsan_2 where danhmuc_khachsan.id=khachsan.danhmuc_id and danhmuc_khachsan_2.id=khachsan.danhmuc2_id  ".(($where!='')?(' and '.$where):'')." Order By ".$Order." Limit ".(($CurrentPage-1)*$PageSize)." , ".$PageSize);
}
//
function khachsan_insert($obj)
{
    return exe_query("insert into khachsan (danhmuc_id,danhmuc2_id,highlights,name,name_url,start,price,dichvu,img,address,phone, email,map,content,title,keyword,description) values ('$obj->danhmuc_id','$obj->danhmuc2_id','$obj->highlights','$obj->name','$obj->name_url','$obj->start','$obj->price','$obj->dichvu','$obj->img','$obj->address','$obj->phone','$obj->email','$obj->map','$obj->content','$obj->title','$obj->keyword','$obj->description')",'khachsan');
}
//
function khachsan_update($obj)
{
    return exe_query("update khachsan set danhmuc_id='$obj->danhmuc_id',danhmuc2_id='$obj->danhmuc2_id',highlights='$obj->highlights',name='$obj->name',name_url='$obj->name_url',start='$obj->start',price='$obj->price',dichvu='$obj->dichvu',img='$obj->img',address='$obj->address',phone='$obj->phone',email='$obj->email',map='$obj->map',content='$obj->content',title='$obj->title',keyword='$obj->keyword',description='$obj->description' where id=$obj->id",'khachsan');
}
//
